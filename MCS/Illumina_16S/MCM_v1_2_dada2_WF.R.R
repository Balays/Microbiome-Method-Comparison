setwd('/mnt/d/Gemini/Microbacontrol')
rm(list = ls(all=T))
gc()
...
###
dir.create('MCM_illumina_v1_2_fastq')
source('copy.fast.R')

#### Metadata, settings and parameters ####

pardir    <- paste0('MCM_illumina_v1_2_fastq'); par.dir <- '/mnt/d/Gemini/Microbacontrol/MCM_illumina_v1_2_fastq'
bamfiles  <- grep('.bai', list.files(pardir, pattern = pattern, recursive = T), value = T, invert = T)
...
metadata$db <- db
metadata$workflow <- 'dada2'
metadata$fastq <- paste0(pardir, '/', metadata$bamfiles)

### metadata and parent fastq directory
indir   <- 'MCM_illumina_v1_2_fastq'

### filtering version
filt.version <- 'filt.v1'

### config
config <- read.delim( paste0('MCM_v1_2_dada2_config_', filt.version, '.tsv'))
...
track.all <- data.frame(NULL)
seq_ids <- unique(metadata$sequencing_ID)
n       <- c(length(seq_ids):1) #  7,
n
for (i in seq_ids[n]) {
  ...
  source('/mnt/d/Gemini/Microbacontrol/MCM_v1_2_dada2.linux.R')
...
write.table(seqtab.all, paste0(filt.version, '_all_sequence.table.tsv'), row.names = F, quote = F, sep='\t')
#### ####
## 
...
### Databases 
dbfile <- c(
  "/mnt/e/data/databases/silva_nr99_v138.1_train_set.fa.gz",
  "/mnt/e/data/databases/silva_species_assignment_v138.1.fa.gz",
  "/mnt/e/data/databases/silva_nr99_v138.1_wSpecies_train_set.fa.gz" )

## OR for EMU db
if (db == 'EMUdb') {
  dbfile <- rep("/mnt/e/data/databases/EMU/species_taxid.dada2.formatted.fasta", 3)
}
...
write.table(spec_pool_silva, paste0(filt.version, '_all_spec_pool_', db, '.tsv'), sep="\t", row.names=T, quote=F)
#write.table(spec_pool_silva, paste0(filt.version, '_all_spec_pool_silva.tsv'), sep="\t", row.names=T, quote=F)

#### ####
## 

source('MCM_v1_2_dada2_WFpart2.R')
