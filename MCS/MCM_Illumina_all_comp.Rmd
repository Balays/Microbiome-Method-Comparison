---
title: "Zymo D3600 MCM V1-V2 : minitax vs EMU vs dada2 -- NCBI vs EMUdb vs SILVA"
author: Balázs Kakuk
output:
   html_document:
      toc: TRUE
...
writetables  <- T
save.Figs    <- F
res.dir      <- 'MCM_V1-V2_Zymo_3600_all_comp'; dir.create(res.dir)
...
## Combine the samples from different sequeincg dates?
combine.samples <- F
save.data  <- 'MCM_V1-V2_Zymo_3600_all_comp_unmerged.RData'
...
- The same samples from different sequencing dates were combined
...
if (all(is.na(mapq.filt))) {
  ...
  ## seq1
  load("minitax_MCM_Zymo_D6330_20221104_Illumina_V1_V2_all_NCBI_genomes/minitax_MCM_Zymo_D6330_Illumina_V1_V2_all_NCBI_genomes.nofilt_PS.RData")
  cols_to_group_by <- c("Vregion", "platform", "sequencing_date", "workflow", "db", "method", "sample_name")
 
  sam_data <- data.frame(ps.minitax.comb@sam_data)
  sam_data$sequencing_date <- '20221104'
  sam_data$sample_name <- gsub('_S.*', '', sam_data$sample)
  sam_data <- unite(sam_data, 'sample', cols_to_group_by, remove = F, na.rm = T)
  sample_names(ps.minitax.comb) <- sam_data$sample
  ps.minitax.comb@sam_data <- sample_data(sam_data)

  ps.ncbi.seq1  <-  glom_samples_var(ps.minitax.comb, cols_to_group_by)
 
  ## seq2
  load("minitax_MCM_Zymo_D6330_20221110_Illumina_V1_V2_all_NCBI_genomes/minitax_MCM_Zymo_D6330_20221110_Illumina_V1_V2_all_NCBI_genomes.nofilt_PS.RData")
  
  sam_data <- data.frame(ps.minitax.comb@sam_data)
  sam_data$sequencing_date <- '20221110'
  sam_data$sample_name <- gsub('_S.*', '', sam_data$sample)
  sam_data <- unite(sam_data, 'sample', cols_to_group_by, remove = F, na.rm = T)
  sample_names(ps.minitax.comb) <- sam_data$sample
  ps.minitax.comb@sam_data <- sample_data(sam_data)

  ps.ncbi.seq2  <-  glom_samples_var(ps.minitax.comb, cols_to_group_by)
  
  ps.ncbi <- merge_phyloseq(ps.ncbi.seq1, ps.ncbi.seq2)
  sam_data <- data.frame(ps.ncbi@sam_data)
  
  ### EMU db 
  load("minitax_MCM_Zymo_D6330_20221104_Illumina_V1_V2_EMUdb/minitax_MCM_Zymo_D6330_Illumina_V1_V2_EMUdb.nofilt_PS.RData")
  cols_to_group_by <- c("Vregion", "platform", "sequencing_date", "workflow", "db", "method", "sample_name")
  ...
  ps.EMUdb.seq1 <- glom_samples_var(ps.minitax.comb, cols_to_group_by)
  
  load("minitax_MCM_Zymo_D6330_20221110_Illumina_V1_V2_EMUdb/minitax_MCM_Zymo_D6330_20221110_Illumina_V1_V2_EMUdb.nofilt_PS.RData")
  cols_to_group_by <- c("Vregion", "platform", "sequencing_date", "workflow", "db", "method", "sample_name")
  ...
  ps.EMUdb.seq2 <- glom_samples_var(ps.minitax.comb, cols_to_group_by)
  
  
  ps.EMUdb <- merge_phyloseq(ps.EMUdb.seq1, ps.EMUdb.seq2)
  sam_data <- data.frame(ps.EMUdb@sam_data)
...
```{r, Import EMU and dada2 on EMUdb}
### Load EMU and dada2 on EMUdb
ps.mcm <- readRDS('D:/Gemini/PS_MCM_EMU_dada2.rds')

ps.mcm <- prune_samples(grep('_v1_2', sample_names(ps.mcm), value=T), ps.mcm)

ps.mcm@sam_data$Vregion  <- 'V1_V2'
ps.mcm@sam_data$platfrom <- 'Illumina'
ps.mcm@sam_data$project  <- 'MCM_Zymo_D3600'
ps.mcm@sam_data <- ps.mcm@sam_data[,c('sample', 'Vregion', 'platform', 'workflow', 'db', 'project')]

colnames(ps.mcm@tax_table) <- c(ranks, 'spec_gen')

otutab   <- data.frame(otu_table(ps.mcm))
taxtab   <- data.frame(tax_table(ps.mcm))
metadata <- data.frame(sample_data(ps.mcm))

taxtab <- taxtab[rowSums(otutab) > 0, ]
otutab <- otutab[rowSums(otutab) > 0, ]

ps.mcm <- phyloseq(otu_table(as.matrix(otutab), taxa_are_rows = T),
                   tax_table(as.matrix(taxtab)),
                   sample_data(ps.mcm))
...
```{r, Import dada2 on SILVA}
### Load dada2 on SILVA
ps.mcm <- readRDS('D:/Gemini/Microbacontrol/MCM.V1-V2.dada2.SILVA_PS.rds')
#ps.mcm <- prune_samples(grep('_v1_2', sample_names(ps.mcm), value=T), ps.mcm)
...
colnames(ps.mcm@tax_table) <- c(ranks, 'ASV_ID')
taxa_names(ps.mcm) <- ps.mcm@tax_table[,"ASV_ID"]

taxtab  <- data.frame(tax_table(ps.mcm))
taxtab  <- unite(taxtab, 'ASV_ID', c('genus', 'species'), sep=' ', remove = F, na.rm = T)
taxtab$species[!is.na(taxtab$species)] <- taxtab$ASV_ID[!is.na(taxtab$species)]

taxtab$genus  [taxtab$species == 'Escherichia-Shigella boydii'] <- 'Shigella'
taxtab$species[taxtab$species == 'Escherichia-Shigella boydii'] <- 'Shigella boydii' 

taxtab$genus  [taxtab$species == 'Escherichia-Shigella coli']   <- 'Escherichia' 
taxtab$species[taxtab$species == 'Escherichia-Shigella coli']   <- 'Escherichia coli' 

taxtab$genus  [taxtab$species == 'Escherichia-Shigella sonnei'] <- 'Shigella' 
taxtab$species[taxtab$species == 'Escherichia-Shigella sonnei'] <- 'Shigella sonnei' 

ps.mcm@tax_table <- tax_table(as.matrix(taxtab[,c(ranks, 'ASV_ID')]))

ps.mcm <- glom_taxa_n(ps.mcm, n = 7, ignore_lineage = F)
taxtab   <- data.frame(tax_table(ps.mcm))
rownames(taxtab)[!is.na(taxtab$species)] <- taxtab$species[!is.na(taxtab$species)]
ps.mcm@tax_table   <- tax_table(as.matrix(taxtab[,c(ranks)]))
taxa_names(ps.mcm) <- rownames(taxtab)

ps.dada2.silva <- ps.mcm
```


```{r, Merge with EMU and dada2}
### Merge
ps.mcm <- merge_phyloseq(ps.dada2.silva, ps.EMUdb)
...
metadata <- metadata %>% dplyr::select(-sample_name)
...
```{r, Combine same samples from direrent sequencing dates }
ps <- ps.all

if (combine.samples) {
  cols_to_group_by <- c("DNA_isolation_method", "Vregion", "platform", "workflow", "db", "method", "sample_nr")
} else {
  cols_to_group_by <- c("DNA_isolation_method", "Vregion", "platform", "sequencing_date", "workflow", "db", "method", "sample_nr")
} 

ps.sum <- glom_samples_var(ps, cols_to_group_by)
ps.sum@sam_data$sample_name <- paste0(ps.sum@sam_data$DNA_isolation_method, ps.sum@sam_data$sample_nr)
...
```{r, omit tax.identity from PS object}
...
plotfun <- function(ps, x='sample_nr', t=t, fill=t) {
  plot_bar(ps, x=x, fill=fill
    ...
    facet_nested(cols   = vars(DNA_isolation_method), #db, workflow),
...
plot_correl <- function(plot_data, cols_to_uniq = c('db', 'workflow', 'method', 'R2')) {
  ...
  facet_nested(rows=vars(db, workflow),
                    #ncol=7, 
               cols=vars(DNA_isolation_method, sample_nr), 
               scales = 'free') +
  labs(
...
plot_means <- plot_sum %>% 
  group_by(across(any_of(c("DNA_isolation_method", "Vregion", "platform", "sequencing_date", "workflow", "db", "method", "rank")))) %>% 
...
## Species level
```{r, Plot Chi-test Species, fig.width=14, fig.height=9, eval=T}
...
gg.chi.spec <- ggplot(plot.merged, 
    ...
    scale_fill_manual(values = pal.man) +
    scale_y_continuous(name=('Chi-stat value')) +
    theme_ipsum() +
...
## Genus level
```{r, Plot Chi-test Genus,   fig.width=14, fig.height=9, eval=T}
