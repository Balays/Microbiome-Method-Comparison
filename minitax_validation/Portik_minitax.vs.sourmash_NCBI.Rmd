---
title: "Portiek et al PacBIO: minitax vs sourmash & NCBI vs proGenomes"
author: Balázs Kakuk
output:
  html_document:
    toc: TRUE
...
misc.dir    <- 'E:/my.R.packages/Rlyeh-main'
minitax.dir <- 'E:/my.R.packages/minitax/R'
...
writetables  <- T
save.Figs    <- F
res.dir      <- 'HiFi_minitax.vs.sourmash'; dir.create(res.dir)
...
GS_nr   <- 1
just_GS <- T
vars <- ('project') # c('DNA_isolation_method', 'db', 'workflow', 'method', 
glom_spec <- T ## glomerate strains from the same species in  the GS?
consider.euk <- T
...
*Comparison of minitax ('BestAln' method) on proGenomes_2, and proGenomes_3 (Host-associated & All representatives) and all NCBI genomes databases with sourmash (k-31) on all NCBI genomes database using  Zymo MCM D6331 PacBio Hifi WGS sequencing data (reported in Portik et al 2022)*
...
- The Gold Standard (Theoretical composition) was: *Zymo 6331*
- The taxonomic lineage of the Gold Standard was taken from *NCBI*
- All species from *Veillonella*, *Faecalibacterium* and *Prevotella* were counted towards *Veillonella rogosae*, *Faecalibacterium prausnitzii* and *Prevotella corporis*, respectively.
- *Bacteroides fragilis*, *Bacteroides fragilis CAG:47* and *Bacteroides fragilis_A* were counted towards *Bacteroides fragilis*
- The correlations (*r2* values) were calculated from the *top20* taxa

- *IS THE PROGENOMES HERE REQUIRED? IT HAS NO EUKARYOTES!*

```{r, Import minitax data}
if (all(is.na(mapq.filt))) {
  
  load("minitax_Portik_etal_2022_PacBio_WGS_all_NCBI_genomes/minitax_Portik_etal_2022_PacBio_WGS_all_NCBI_genomes.nofilt_PS.RData")
  ...
  #load("minitax_Portik_etal_2022_PacBio_WGS_proGcontigs_3.repres/minitax_Portik_etal_2022_PacBio_WGS_proGcontigs_3.MAPQ.filt_PS.RData")
  #ps.prog3.repres <- ps.minitax.comb
...
```{r, Import sourmash data (KRONA), eval=F}
ps <- readRDS("E:/data/Portik_etal_2022/sourmash.NCBI_PS.rds")
ps.sourmash.NCBI <- ps

ps <- readRDS("E:/data/Portik_etal_2022/sourmash.proG_PS.rds")
ps.sourmash.proG_2 <- ps
sample_data(ps.sourmash.proG_2)$db <- 'proGcontigs_2'

ps.sourmash <- merge_phyloseq(ps.sourmash.NCBI, 
                              ps.sourmash.proG_2)
sample_data(ps.sourmash)$method <- 'sourmash'

# sample_data(ps.sourmash)

```


```{r, Import sourmash data (Kreport), eval=T}
ps.sourmash <- readRDS('ps.portik.sourmash.RDS')

ps.sourmash@sam_data$platform <- 'PacBio'
ps.sourmash@sam_data$Vregion  <- 'WGS'
ps.sourmash@sam_data$db       <- c('all_NCBI_genomes', 'proGcontigs_2')

ps.sourmash <- prune_samples(rownames(ps.sourmash@sam_data[ps.sourmash@sam_data$db == 'all_NCBI_genomes']),
                             ps.sourmash)

```


```{r, Merge sourmash with minitax}
...
```{r, Exclude taxa below a threshold}
...
```{r, Fix sample names and metadata, eval=F}
metadata <- sample_data(ps.all)
...
metadata <- data.frame(metadata,
                       sequencing_date = '2022111104',
                       #Vregion         = 'V1_2',
                       DNA_isolation_method = stri_extract_last_regex(metadata$sample, paste0(c('MN', 'Q', 'Z', 'I'), collapse = '|')),
                       #sample_name   = stri_extract_first_regex(metadata$sample, paste0(metadata$sample, '[0123456789]') ),
                       sample_nr     = as.integer(gsub('[^0123456789]', '', metadata$sample))
)
metadata$DNA_isolation_method <- factor(metadata$DNA_isolation_method)
metadata$Vregion <- factor(metadata$Vregion)
metadata$sample_name <- factor(metadata$sample)
metadata$sequencing_date <- factor(metadata$sequencing_date)

## regenerate sample names
IDcols <- c("sample_name", "sequencing_date", "workflow", "db", "method")
...
```{r, eval=FALSE}
metadata.gs         <- metadata[1:GS_nr, ]
metadata.gs$project <- GS_name

metadata.gs <- unite(metadata.gs, 'sample', vars, remove = F)
metadata.gs[,c('sample', vars)]

metadata.gs$sample_name <- GS_name
metadata.gs$sample_nr   <- GS_name

rownames(metadata.gs) <- metadata.gs$sample

if(just_GS) {
  metadata.gs[,] <- GS_name
}
metadata.gs$sample <- rownames(metadata.gs)

otutab.gs <- data.frame(NULL)
for (i in 1:nrow(metadata.gs)) {
  otutab.gs <- plyr::rbind.fill(otutab.gs, data.frame(zymo.GS, sample=metadata.gs$sample[i]))
}
otutab.gs <- as.data.frame(otutab.gs[,c('species', 'count', 'sample')] %>% spread(sample, count, fill=0))
rownames(otutab.gs) <- otutab.gs[,1]

otutab.gs <- otutab.gs %>% select(any_of(metadata.gs$sample))
...
gg.ds <- ggplot(
  all.sample.stat.df.gt[
   ...
   !is.element(all.sample.stat.df.gt$statistic, c('True_positives', 'False_positives', 'False_negatives', 'Chi_stat', 'Chisq.test.p_value', 'Chisq.test.sign'))
    , ]) +
    geom_col(aes(x=method, y=value, fill=db), position=position_dodge2()) +
    #geom_jitter(aes(x=workflow, y=value, color=DNA_isolation_method)) +
    #geom_point(aes(x=workflow, y=value, color=statistic)) +
  scale_fill_manual(values = pal.man) +
  coord_cartesian(ylim = c(0,1)) +
  ...
  facet_nested(cols=vars(Detection_threshold_percent, workflow, method), 
...
samples.to.keep <- plyr::rbind.fill(
  ...
  metadata[#metadata$workflow == 'minitax'  & 
           #metadata$db == 'ProGenomes'
           , ]
             
)$sample
  
ps <- ps.all # prune_samples(samples.to.keep, ps.all)
...
plotfun <- function(ps, x='sample_name', t=t, fill=t) {
  plot_bar(ps, x=x, fill=fill
    ...
    facet_nested(cols   = vars(db, workflow, method), scales = 'free_x',
                 rows   = vars(project) )
}
...
#cowplot::plot_grid(ggtop20, ggtop20.GS, nrow = 1, rel_widths = c(4, 1))
...
plot_correl <- function(plot_data) {
  ggplot(plot_data, aes(x = Observed, y = Theoretical, color = Taxa)) +
  geom_point() +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "red") +
  geom_label(data = data.frame(unique.data.frame(plot_data[,c('db', 'workflow', 'method', 'R2')]), x=0, y=1, Taxa=''),
  ...
  theme_ipsum() + 
  theme(legend.position="right") +
  facet_nested(rows=vars(workflow, method), cols=vars(db), scales = 'free') +
  labs(
...
```{r, R2 sum, fig.width=6, fig.height=9}
# Create a scatter plot with R-squared values
...
plot_means <- plot_sum %>% 
  group_by(across(any_of(c(colnames(metadata), "rank")))) %>% 
  summarise(R2_mean=mean(R2))

ggplot(plot_sum, aes(x = workflow, y = R2, fill = workflow)) +
  ...
  facet_nested(cols=vars(db, workflow, method),
               rows=vars(rank),
             # ncol=5, 
               scales = 'free')
...
## Species level
```{r, Plot Chi-test Species, fig.width=30, fig.height=12}
...
gg.chi <- ggplot(plot.merged, 
                      aes(x=workflow, y=value.x, fill=workflow) ) +
    geom_col(position=position_dodge2()) +
...
## Genus level
```{r, Plot Chi-test Genus, fig.width=12, fig.height=10}
...
cowplot::plot_grid(gg.chi, gg.chi.gen, ncol = 2)
```
